...
function StoryBoard(props) {
    ...
    // save to "this" to remove "load" handler on unmount
        ...
        // reader.readAsDataURL(bate);
        // reader.onload = function() {
        //     console.log(reader.result)
        // }
        let image = <img src={require('./bate.jpg').default}></img>
        setImage(image)
    }
    ...
    if(store.mode === "comic") {
        ...
        <Box sx={{ height: '80vh', position: 'relative', backgroundColor: 'white', borderRadius: '10px', width: '70%', left: '15%'}}>
            <Editor tool={tool} setTool={setTool} lines={lines} setLines={setLines} trash={trash} setTrash={setTrash} isDrawing={isDrawing} stageRef={stageRef} stage={stage} image={image}/>
        </Box>
    }
    else {
